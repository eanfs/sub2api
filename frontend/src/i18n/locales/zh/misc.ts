export default {
  ...
  payment: {
    ...
    admin: {
      ...
      forceFinalizeRefund: '强制终结退款',
      orderNo: '订单编号',
      forceFinalizeRefundTitle: '强制终结退款',
      forceFinalizeRefundMessage: '网关查询无法给出结论时使用。请先在网关后台确认该笔退款是否实际发生，再选择对应结果。',
      forceFinalizeRefunded: '网关已退款（扣回用户余额并标记为已退款）',
      forceFinalizeNotRefunded: '网关未退款（标记为退款失败，可重试）',
      refundInfo: '退款信息',
      refundEnabled: '允许退款',
