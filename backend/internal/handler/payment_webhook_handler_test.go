...
func TestAntomWebhookAcknowledgement(t *testing.T) {
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	writeSuccessResponse(c, payment.TypeAntom)
	...
	require.Equal(t, "SUCCESS", body.Result.Code)
	require.Equal(t, "S", body.Result.Status)
}
...
func TestExtractOutTradeNo(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "antom payment request selects original instance",
			providerKey: payment.TypeAntom,
			rawBody:     `{"notifyType":"PAYMENT_RESULT","paymentRequestId":"sub2_antom_123","paymentId":"upstream_123"}`,
			want:        "sub2_antom_123",
		},
	}
