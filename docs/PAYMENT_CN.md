...
| **Antom** | 卡、钱包等已签约方式 | 托管收银台，多币种，支持退款及取消 |
...
完成 Antom 商户签约后，在服务商管理中添加 **Antom**，并在支付方式中启用 **Antom**。前台提供独立按钮，跳转 Antom 托管收银台选择签约的卡或钱包。

| 参数 | 说明 |
|------|------|
| Client ID | Antom Dashboard 的应用标识；沙箱使用对应的 SANDBOX_ 标识 |
| 商户私钥 | RSA 2048 位以上，PKCS#1 / PKCS#8，PEM 或 Base64 DER；后台读取时脱敏，编辑留空保留 |
| Antom 公钥 | Antom 提供的公钥，不是上传给 Antom 的商户公钥 |
| 密钥版本 | 默认 1，与商户签名密钥版本一致 |
| API 基础地址 | 默认 https://open-sea-global.alipay.com；北美使用 open-na-global.alipay.com，欧洲使用 open-de-global.alipay.com，美国商户使用 open.antglobal-us.com；均为 HTTPS 根地址 |
| 支付币种 | 默认 CNY，可选择账户已开通的币种；系统不会自动换汇 |
| 结算币种 | 可选三位币种代码；留空使用商户默认值 |
| 异步通知地址 | 公网 HTTPS 站点 + /api/v1/payment/webhook/antom；反向代理不得改写签名路径或请求体 |
| 支付完成回跳地址 | 公网 HTTPS 站点 + /payment/result。Antom 要求该地址必填；服务端到服务端下单不会自带回跳地址，因此必须在此配置兜底 |

系统通过 createPaymentSession 创建托管会话，并自动传入通知地址、用户标识及支付完成回跳地址。会话创建成功、浏览器回跳以及银行卡授权成功都不直接入账；只有一次性结算的支付方式（当前为支付宝中国钱包）凭支付通知入账，其余方式（含卡、Apple Pay、Google Pay 以及未识别的方式）都必须有成功且覆盖全额的扣款交易。普通查单在五分钟通知等待期后兜底，显式取消前仍立即查单。

支持退款、退款查询及取消；退款处理中或结果未知时仅查询原退款，不能重新发起。退款标识落库在订单上，不依赖审计日志。确认失败后重试使用新的退款尝试标识；查询校验含手续费的网关退款金额，但不额外扣减用户余额。网关查询无法给出结论时（例如退款金额不符），可在订单列表对 REFUND_PENDING 订单使用「强制终结退款」，按网关后台的实际结果标记为已退款或退款失败。存在待支付订单时禁止更换商户、密钥、网关及币种。停用实例后仍可接收历史订单的扣款通知，不要提前删除实例或轮换验签公钥。

上线前使用匹配的沙箱 Client ID 和密钥，在对应地区网关验证实际支付、扣款通知及退款。协议细节见 [Antom 官方托管收银台指南](https://docs.antom.com/ac/cashierpay/HOSTEDCKP.md)。
